		...
		<div class="pb-left-column col-xs-12 col-sm-4 col-md-5">
			...
			<div id="image-block" class="clearfix">
				...
				{elseif $product->specificPrice && $product->specificPrice.reduction && $productPriceWithoutReduction > $productPrice}
					<span class="offercount-product">{if $product->specificPrice && $product->specificPrice.reduction_type == 'percentage'}-{$product->specificPrice.reduction*100}%{/if}</span>
				{/if}
				{if $have_image}
										...
										<p class="our_price_display" itemprop="offers" itemscope itemtype="https://schema.org/Offer">{strip}
											...
											<span id="old_price"{if (!$product->specificPrice || !$product->specificPrice.reduction)} class="hidden"{/if}>{strip}
												{if $priceDisplay >= 0 && $priceDisplay <= 2}
													...
													<span id="old_price_display"><span class="old-price">{if $productPriceWithoutReduction > $productPrice}{convertPrice price=$productPriceWithoutReduction|floatval}{/if}</span></span>
												{/if}
											{/strip}</span>
										</p>
							...
							<div class="product_attributes clearfix">
								...
								{if !$PS_CATALOG_MODE}
									<p id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none;"{/if}>
										...
										<div{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{else} class="r-product-page-buy-btn" {/if}>
											<p id="add_to_cart" class="buttons_bottom_block no-print">
												<button type="submit" name="Submit" class="r-product-page-buy-btn-skin">
													<i class="icon-shopping-cart"></i>
													<span>{if $content_only && (isset($product->customization_required) && $product->customization_required)}{l s='Customize'}{else}Купить{/if}</span>
												</button>
											</p>
										</div>
