...
class PosPaymentModule extends PaymentModule
{
     ...
     * @param boolean   $dont_touch_amount
    ...
    {
        $payment_method = $payment_method; // unused validate prestashop
        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            $package_list = $this->context->cart->getPackageList(true, true);
            ...
            foreach ($package_list as $id_address => $package_by_address) {
                foreach ($package_by_address as $id_package => $package) {
                    ...
                    $order->payment = !empty($payment) ? $payment : $this->l('FREE', basename(__FILE__, '.php'));
                    ...
                    // check version assign mobile_theme
                    ...
                    // Insert order id into table pos_orders
                    Db::getInstance()->insert('pos_orders', array(
                        'id_pos_order' => (int) $order->id,
                        'note' => pSQL($this->context->cookie->note),
                        'show_note' => (int) $this->context->cookie->show_note_on_invoice,
                    ...
                    if (version_compare(_PS_VERSION_, '1.5.5.0') < 0) {
                        if (empty($package_list[$id_address][$id_package]['id_warehouse'])) {
                            $package_list[$id_address][$id_package]['id_warehouse'] = 0;
                        }
                    }
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (!$order_creation_failed && isset($order->id)) {
                    ...
                    if (Configuration::get('PS_STOCK_MANAGEMENT') && $order_detail->getStockState()) {
                        ...
                        // add id_employee to order_history
                        $history->id_employee = (int) $this->context->cookie->id_employee;
                    ...
                    // add id_employee to order_history
                    $new_history->id_employee = (int) $this->context->cookie->id_employee;
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id && Configuration::get('POS_SEND_EMAIL_TO_CUSTOMER')) {
                        if ((int) $this->context->customer->id !== (int) $this->getDefaultCustomerId()) {
                    ...
                    if (Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT')) {
                        ...
                        foreach ($product_list as $product) {
                            ...
                            if (StockAvailable::dependsOnStock((int) $product['product_id'])) {
                                StockAvailable::synchronize((int) $product['product_id'], (int) $order->id_shop); // synchronizes
                            }
                        }
                    }
                } else {
    ...
    protected function getFileAttachement(PosOrder $order, OrderState $order_state)
    {
        ...
        if ((int) Configuration::get('PS_INVOICE') && $order_state->invoice && $order->invoice_number) {
            ...
            $pdf = new PosPDFInvoice($invoices_collection, PosPDF::TEMPLATE_INVOICE, $this->context->smarty, '');
